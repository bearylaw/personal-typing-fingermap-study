...
def fig_sfb(profile, test):
    ...
    cols = [S3 if n == "QWERTZ" else (S1 if n in ("ZEHN", "MECH") else S2) for n in names]
...
def fig_fingerload(profile, test):
    ...
    for i, name in enumerate(picks):
        ...
        col = S3 if name == "QWERTZ" else (S1 if name in ("ZEHN", "MECH") else S2)
        alpha = 1.0 if name in ("QWERTZ", "MECH", "ZEHN") else 0.85
        ax.bar(x + i * width - 0.4 + width / 2, share, width * 0.9,
               color=col, alpha=alpha, label=name)
